...
let arrSize = landWidth * landHeight;

let cells = Array.from(Array.arrSize);
for (let i = 0; i < arrSize; i++) {
  arr[i] = [horisontal, vertical, value];
}
/*
...
const land = new THREE.Mesh(
  new THREE.PlaneBufferGeometry(landWidth, landHeight),
  material
);
...
const object = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({ color: 0xfa70aa })
);
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.keyCode == 49) {
    for (let i = 0; i < cells.length; i++) {
      if (
        cells[i][0] == object.position.x &&
        cells[i][1] == object.position.z
      ) {
        if (cells[i][2] == 0) {
...
function landGrow(i) {
  if (Math.abs(cells[i][0]) + 10 > landWidth / 2) {
    landWidth += cells.length / 2 - Math.abs(cells[i][0]) + 10;
    console.log(landWidth);
    console.log(landHeight);
    landHeight += cells.length / 2 - Math.abs(cells[i][0]) + 10;
    console.log(landWidth);
    console.log(landHeight);
  }
  if (Math.abs(cells[i][1]) + 10 > landHeight / 2) {
    landWidth += cells.length / 2 - Math.abs(cells[i][1]) + 10;
    console.log(landWidth);
    console.log(landHeight);
    landHeight += cells.length / 2 - Math.abs(cells[i][1]) + 10;
    console.log(landWidth);
    console.log(landHeight);
  }
  console.log(landWidth);
  ...
  for (let i = 0; i < cells.length; i++) {
    for (let j = 0; j < cells.length; j++) {
      if (cells[i][j][2] == 2) {
        ...
        road.position.set(cells[i][0], y + 0.01, cells[i][1]);
        scene.add(road);
      }
    }
  }
